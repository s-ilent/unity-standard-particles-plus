...
// Light Volumes support -- Silent
#include "LightVolumes.cginc"

struct SurfaceOutputStandardLV
{
...
inline half4 LightingStandardLV (SurfaceOutputStandardLV s, float3 viewDir, UnityGI gi)
{
...
inline void LightingStandardLV_GI (
    SurfaceOutputStandardLV s,
    UnityGIInput data,
    inout UnityGI gi)
{
    ...
    #if defined(USPP_VRC_LIGHT_VOLUMES_FULL)
    float3 L0; float3 L1r; float3 L1g; float3 L1b;
    LightVolumeSH(data.worldPos, L0, L1r, L1g, L1b);

    gi.indirect.specular += LightVolumeSpecularDominant(s.Albedo, s.Smoothness, s.Metallic, s.Normal, data.worldViewDir, 
        L0, L1r, L1g, L1b);
    gi.indirect.diffuse = LightVolumeEvaluate(s.Normal, L0, L1r, L1g, L1b);
    #else
    gi.indirect.diffuse = LightVolumeSH_L0(data.worldPos);
    #endif
}

#define SurfaceOutputStandard SurfaceOutputStandardLV
...
void surf (Input IN, inout SurfaceOutputStandardLV o)
{
